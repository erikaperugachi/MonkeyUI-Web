	...
	var monkeyUI = new function () {
	    ...
	    function drawInput(content, input) {
	        ...
	        if (input.isAttachButton) {
	            _html = _html + '<div class="button-input">' + '<button id="button-attach-file" class="button-icon"></button>' + '<input type="file" name="attach" id="attach-file" style="display:none" accept=".pdf,.xls,.xlsx,.doc,.docx,.ppt,.pptx, image/*">' + '</div>';
	        }

	        if (input.isAudioButton) {
	            _html = _html + '<div class="button-input">' + '<button id="button-cancel-audio" class="button-icon"></button>' + '</div>';
	        }

	        _html = _html + '<textarea id="message-text-input" class="textarea-input" placeholder="Write a secure message"></textarea>';

	        if (input.isAudioButton) {
	            _html = _html + '<div id="record-area" class="disappear">' + '<div class="record-preview-area">' + '<div id="button-action-record">' + '<button id="button-start-record"></button>' + '</div>' + '<div id="time-recorder"><span id="minutes">00</span><span>:</span><span id="seconds">00</span></div>' + '</div>' + '</div>';
	        }

	        if (input.isSendButton) {
	            _html = _html + '<div class="button-input">' + '<button id="button-send-message" class="button-icon"></button>' + '</div>';
	        }

	        if (input.isAudioButton) {
	            _html = _html + '<div class="button-input">' + '<button id="button-record-audio" class="button-icon"></button>' + '</div>';
	        }

	        if (input.isEphemeralButton) {
	            _html = _html + '<div class="button-input">' + '<button id="button-send-ephemeral" class="button-icon timer_icon"></button>' + '</div>';
	        }
